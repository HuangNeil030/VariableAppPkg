...
DataSize：輸入時表示 Buffer 大小；輸出時表示變數實際資料大小。
...
VariableName：輸入為上一個變數名稱 (或空字串開始)；輸出為下一個變數名稱。
...
處理緩衝區不足：若回傳 EFI_BUFFER_TOO_SMALL，需 ReallocatePool 擴大緩衝區並重試。
...
GetNextVariableName 迴圈掃描所有變數。

將變數資訊 (Name, Guid, Size) 存入動態陣列 (VarList)。
...
GetNextVariableName 迴圈掃描。

使用 StrStr 檢查名稱是否包含關鍵字。

若符合，呼叫 GetVariable 讀取資料並 PrintVariableData (綠色 Hex Dump)。
